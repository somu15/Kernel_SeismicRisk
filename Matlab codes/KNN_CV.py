...
X_plot = np.log(np.linspace(0.01, 2, 10)[:, None])
...
err_rc = np.zeros(len(samps_space))

N = 30

for iii in np.arange(0,N,1):
    X_train, X_test, y_train, y_test = train_test_split(lnIM, res_knn, test_size = 0.33, random_state = rnds[iii])
    ...
    for kk in np.arange(0,len(samps_space),1):
        ...
        err_rc[kk] = err_rc[kk] + np.sum((std_test-std_train)*(std_test-std_train))
    print(iii/N)
...
samps_space = np.arange(10,60,1)
err_smf = np.zeros(len(samps_space))
...
for iii in np.arange(0,N,1):
    ...
    for kk in np.arange(0,len(samps_space),1):
        ...
        err_smf[kk] = err_smf[kk] + np.sum((std_test-std_train)*(std_test-std_train))
...
samps_space = np.arange(10,60,1)
err_wood = np.zeros(len(samps_space))
...
for iii in np.arange(0,N,1):
    ...
    for kk in np.arange(0,len(samps_space),1):
        ...
        err_wood[kk] = err_wood[kk] + np.sum((std_test-std_train)*(std_test-std_train))
